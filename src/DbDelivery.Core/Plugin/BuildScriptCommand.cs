...
namespace DbDelivery.Core.Plugin {
    ...
    public class BuildScriptCommand : AbstractDatabasePluginCommand {
        ...
        public override bool Execute() {
            ...
            IList<MigrationScriptModel> appliedScripts = GetAppliedScripts(encoding);
            ...
            if (!String.IsNullOrEmpty(actualScript)) {
                BuildActualScript(actualScript, notAppliedScripts, encoding);
            }
            return true;
        }

        private void BuildActualScript(string actualScript, IList<string> notAppliedScripts, Encoding encoding) {
            ...
            File.WriteAllText(actualScriptFile, "--=============== Not Applied Scripts: ===============" + Environment.NewLine + Environment.NewLine, encoding);
            File.AppendAllText(actualScriptFile, "-- " + String.Join(Environment.NewLine + "-- ", notAppliedScripts) + Environment.NewLine + Environment.NewLine, encoding);

            File.AppendAllText(actualScriptFile, "--=============== Insert History Scripts: ===============" + Environment.NewLine + Environment.NewLine, encoding);
            ...
            File.AppendAllText(actualScriptFile, historyScript + Environment.NewLine + Environment.NewLine, encoding);
            ...
            foreach (string item in notAppliedScripts) {
                ...
                File.AppendAllText(actualScriptFile, Environment.NewLine + String.Format(headerSriptLine, "BEGIN SCRIPT", filename), encoding);
                string content = File.ReadAllText(item, encoding);
                File.AppendAllText(actualScriptFile, content, encoding);
                File.AppendAllText(actualScriptFile, String.Format(headerSriptLine, "END SCRIPT", filename), encoding);
            }
        }
        ...
        private List<MigrationScriptModel> GetAppliedScripts(Encoding encoding) {
            using (DbConnection connection = GetConnection()) {
                using (DbCommand cmd = connection.CreateCommand()) {
                    ...
                    string script = File.ReadAllText(scriptPath, encoding);
